...
Genetic ancestry is inferred using the Gloyn-lab [GeneticAncestry pipeline](https://github.com/gloynlab/GeneticAncestry). Primary ancestry probabilities are reported on the outer ring and secondary (sub-ancestry) probabilities on the inner ring, aligned with the 1000 Genomes Project primary populations. See the [1000 Genomes population descriptions](https://www.internationalgenome.org/dataportal/population) for the reference panels used.

## Genetic Risk Scores

*Current coverage: 404 donors.*

Polygenic risk scores are reported for both type 1 and type 2 diabetes.

**T1D Genetic Risk Score (T1D-GRS).** Multiple T1D-GRS variants are computed: the 67-variant standardized score from Luckett et al. ([2025](https://pubmed.ncbi.nlm.nih.gov/40267362/), *Diabetes Care*); the GRS2 PRS with 4 African-ancestry SNPs from Qu et al. ([2022](https://pubmed.ncbi.nlm.nih.gov/34997821/), *Pediatric Diabetes*); and the TOPMED-R2 version of GRS2 from Sharp et al. ([2019](https://pubmed.ncbi.nlm.nih.gov/30655379/), *Diabetes Care*).

**T2D Genetic Risk Score (T2D-GRS).** Partitioned polygenic scores are computed under two complementary frameworks: a soft-clustering framework in which variants contribute to multiple clusters with varying weights (Smith et al. [2024](https://pubmed.ncbi.nlm.nih.gov/38443691/), *Nature Medicine*), and a hard-clustering framework in which each variant is uniquely assigned to a single cluster (Suzuki et al. [2024](https://pubmed.ncbi.nlm.nih.gov/38374256/), *Nature*); Udler et al. (2018) provides the earlier soft-clustering basis.

Both ancestry and GRS data were contributed by the lab of Dr. Anna Gloyn (Stanford); see the [Contributors](https://dev.humanislets.com/contributors) page.
